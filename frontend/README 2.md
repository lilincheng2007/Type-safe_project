...
src/
├── apis/                    # APIMessage 封装
│   ├── admin/              # 管理员入驻审核、平台促销、退款仲裁 API
│   ├── ai/                 # AI 搜索、AI 周报、AI 文案、经营建议 API
│   ├── merchant/           # 商家、目录、店铺、商品、图片、订单/退款 API
│   ├── order/              # 顾客订单、结算、取消、确认完成、聊天、退款 API
│   ├── review/             # 评价、回复、点赞、评价图片 API
...
- `ProfileTab.tsx`：钱包、吃货等级、优惠券、收藏、待收货订单、历史订单、退款/评价入口、AI 饮食周报。
...
- `OrdersTab.tsx`：待商家接单、接单/拒收、制作中和出餐完成。
- `MerchantReviewsTab.tsx`：评价回复、退款申请处理。
- `ProfileTab.tsx`：店铺资料、公告、描述、促销、图片和评价概览。
...
- 入驻申请审核：通过后店铺写入正式商家档案，驳回需记录原因。
- 平台促销配置：顾客结算时由后端统一校验和计算。
- 退款仲裁：商家驳回后，管理员可最终通过或驳回。

### 订单聊天页：`src/pages/OrderChatPage/`

- 顾客、商家、骑手按角色进入 `/delivery/chat/:orderId?peer=...`。
- 文本和图片消息走拆分后的角色 API；未读数由 `useOrderChatUnreadCounts` 轮询后端。
- `src/apis/order/OrderChatClient.ts` 仅按当前角色选择具体 API 文件，不定义业务 API class。
...
| `merchantorderacceptapi` / `merchantorderrejectapi` / `merchantorderreadyapi` | 接单、拒收、出餐完成 |
| `merchantrefundrequestsapi` / `merchantrefundacceptapi` / `merchantrefundrejectapi` | 商家处理退款申请 |
| `merchantstoreonboardingrequestsapi` | 查看入驻申请状态 |
...
| apiName | 用途 |
|---|---|
| `merchantreviewsapi` | 查询店铺评价 |
| `customersubmitorderreviewapi` | 顾客提交订单评价 |
| `customerreviewvoteapi` | 顾客点赞/取消点赞评价 |
...
| apiName | 用途 |
|---|---|
| `adminstoreonboardingrequestsapi` | 获取入驻申请 |
| `adminstoreonboardingacceptapi` / `adminstoreonboardingrejectapi` | 审核入驻申请 |
| `adminrefundrequestsapi` | 获取退款仲裁队列 |
| `adminrefundacceptapi` / `adminrefundrejectapi` | 处理退款仲裁 |
| `adminplatformpromotionsapi` / `adminplatformpromotionsupdateapi` | 查询/保存平台促销 |
...
- `use-customer-portal-store.ts`
  - 保存顾客档案、商家目录、商品、购物车、收藏、钱包余额、订单、评价、退款、AI 周报和 AI 订单文案。
  - 提供刷新、加购、改数量、结算、取消、确认完成、充值、舍弃过期券、保存收货联系人、再来一单、退款申请、评价提交、生成 AI 周报等动作。
  - `refreshPortal` 做 in-flight 去重，并并行加载顾客档案、目录和订单。
- `use-merchant-console-store.ts`
  - 保存商家账号、店铺列表、当前店铺和标签页。
  - 提供刷新、创建店铺申请、接单/拒收/出餐、创建/更新商品、保存店铺公告/促销/描述、更新/上传店铺图等动作。
- `use-rider-app-store.ts`
  - 保存骑手账号、可抢订单、配送状态。
...
- 新增业务接口时，在 `src/apis/<module>/` 增加一个对应 `XxxAPI.ts` 文件；每个业务 API 文件只定义一个 `APIMessage` class。
- 需要按当前角色选择多个 API 时，使用不以 `API.ts` 结尾的 client/helper 文件聚合调用，例如 `src/apis/order/OrderChatClient.ts`。
- 新增领域对象时，在 `src/objects/<module>/` 增加同名对象文件；新增请求/响应包装类型时放入 `src/objects/<module>/apiTypes/`，并同步后端 `objects` / `objects/apiTypes`。
- 不在前端伪造真实业务结果；涉及下单、取消、充值、保存资料、抢单、接单、出餐、退款、评价等操作必须调用后端 API。
- 会话本地只保存 JWT、账号、角色和登录时间；收藏、通知快照等 UI 偏好可本地缓存，订单/钱包/退款等真实业务数据必须通过 API 刷新。
