...
一个类型安全的外卖平台全栈项目，覆盖顾客、商家、骑手、管理员四类角色。前端使用 Vite + React + TypeScript，后端使用 Scala 3 + http4s + Circe + PostgreSQL。前端只通过统一 `APIMessage` 网关访问后端，业务数据以后端数据库为准。

## 功能概览

### 顾客端

- 多角色账号体系中的顾客注册、登录、JWT 会话。
- 浏览商家目录、进入店铺点餐、维护购物车和收藏。
- 结算页选择收货联系人、优惠券、平台/店铺促销并提交订单。
- 钱包充值、余额校验、仅商家未接单前取消退款。
- 多组收货联系人管理，必须且只能有一组默认联系人。
- 优惠券抵扣、过期券灰色提示和“含泪舍弃”。
- 吃货积分与等级：确认完成订单后按实付金额累计积分，升级发券。
- 待收货/历史订单、订单详情、确认完成、再来一单。
- 订单备注图片、订单聊天、未读消息提示。
- 订单评价、评价图片、评价点赞、退款申请与仲裁结果反馈。
- AI 智能搜索、AI 饮食周报、AI 订单进度叙事文案。

### 商家端

- 商家登录后管理一个或多个店铺。
- 店铺资料、店铺公告、店铺描述、店铺图片 URL 或文件上传。
- 新店入驻申请由管理员审核；店铺促销可由商家维护。
- 商品创建、编辑、图片、库存、上下架和库存状态。
- 商家手动接单或拒收；接单后制作，完成出餐后进入骑手可抢单流程。
- 订单聊天、评价回复、退款申请处理。
- AI 店铺描述、AI 菜品描述和经营建议生成。
...
│       ├── pages/               # 登录、注册、顾客、商家、骑手、管理员、订单聊天页面
│       ├── stores/              # Zustand 页面状态
...
└── backend/
    ...
    └── src/
        ...
        ├── admin/               # 管理员审核、平台促销、退款仲裁
        ├── user/                # 登录注册、顾客资料、钱包、优惠券
        ├── merchant/            # 商家、店铺、商品、目录、图片、退款处理
        ├── order/               # 下单、订单、结算、取消、确认完成、聊天、退款
        ├── review/              # 评价、回复、投票、评价图片
        ├── rider/               # 骑手、抢单、配送状态、免责卡
        ├── ai/                  # AI 搜索、周报、文案生成、经营建议
...
- 新增业务 API 时，后端 `XxxAPIMessage.scala` 与前端 `src/apis/**/XxxAPI.ts` 必须一一对应；每个业务 API 文件只定义一个 API class。
- 领域对象放 `objects/`，请求/响应对象放 `objects/apiTypes/`。
